...
def overlay_ffts_in_directory(directory, save_filename='overlay_fft.png', smooth_window=10):
    ...
    heights = [extract_height(filename) for filename in filenames]
    markers = ['o', 's', '^', 'x', '+']  # Marker styles for different heights
    for filename, height, marker in zip(filenames, heights, markers):
        if filename.endswith('.wav'):
            ...
            plt.plot(frequencies, smoothed_magnitudeFFT_dB, label=f"{clean_title(filename)}", alpha=0.7, marker=marker)
